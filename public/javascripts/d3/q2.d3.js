 ...
 * @param {*} data 
 */
function displayQ2MultilineGraph(data){
    ...
    const margin = {top: 50, right: 250, bottom: 30, left: 100};
    ...
    let svg = canvas('#q2container', width, height);
    // content dimensions
    ...
    // Append graph title
    svg.append('text').html('Home court performance of each team over time')
        .attr('fill', 'white')
    ...
    y.domain([0, 100]);
    z.domain(teams.map(t => { return t.id }));
    ...
    legend.append('text')
        ...
        .attr('fill', d => { let col = d.country == '0' ? 'white' : '#f9e89a'; return col; })
    ...
    function mouseclick(d){
        ...
        let col = d.country == '0' ? 'white' : '#f9e89a';
        $(`.${id} text`).css('fill', toggled ? col : '#898a8c');
